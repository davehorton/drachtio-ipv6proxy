...
CallProcessor.prototype.start = function() {
  this._srf.invite( ( req, res ) => {
    ...
    var hostport = require('../app').hostport ;

    var host = parseUri( req.uri ).host ;
    if( -1 !== hostport.indexOf( host ) ) {
      console.log(`rejecting spam INVITE sent directly to us at address ${host}`);
      return res.send(603);
    }
